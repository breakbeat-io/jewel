...
struct DiscTrackList: View {
    ...
    var body: some View {
        ...
        let tracks = userData.collection.slots[slotId].album?.relationships?.tracks.data
        let discTracks = tracks?.filter { $0.attributes?.discNumber == discNumber }
        let albumArtist = userData.collection.slots[slotId].album?.attributes?.artistName
